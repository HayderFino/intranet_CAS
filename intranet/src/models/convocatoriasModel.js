...
class ConvocatoriasModel {
  constructor() {
    this.htmlPath = path.join(
      __dirname,
      "../../header_menu/cas/convocatorias.html",
    );
    this.baseDataDir = path.join(
      __dirname,
      "../../data/menu header/la cas/talento humano/Convocatorias",
    );
  ...
  getAll() {
    ...
    // Regex robusto para capturar cada tarjeta de convocatoria
    const itemRegex =
  ...
  create(item) {
    ...
    const relativePath = `../../data/menu header/la cas/talento humano/Convocatorias/${item.filename}`;
    ...
    const gridTrigger = 'id="convocatorias-grid"';
  ...
  update(id, newData) {
    ...
    if (match) {
      ...
      const href = newData.filename
        ? `../../data/menu header/la cas/talento humano/Convocatorias/${newData.filename}`
  ...
  delete(id) {
    ...
    if (match) {
      ...
      if (
        ...
        const relativePath = decodeURIComponent(hrefMatch[1].split("?")[0]);
...
module.exports = new ConvocatoriasModel();
