...
class PlanMonitoreoModel {
  constructor() {
    this.htmlPath = path.join(
      __dirname,
      "../../header_menu/cas/plan-monitoreo-sigep.html",
    );
    // Carpeta física según solicitud
    this.dataDir = path.join(
      __dirname,
      "../../data/menu header/la cas/talento humano/Plan de monitoreo del SIGEP",
    );
  }
  ...
  getAll() {
    ...
    // Buscamos el contenido dentro del div con id="plan-monitoreo-grid"
    const gridRegex = /id="plan-monitoreo-grid"[\s\S]*?>([\s\S]*?)<\/section>/i;
      ...
      while ((cardMatch = cardRegex.exec(gridContent)) !== null) {
        ...
        if (idMatch) {
          items.push({
            ...
            category: "Plan de Monitoreo del SIGEP",
          });
        }
      }
    }
  ...
  create(item) {
    ...
    const relativePath = `../../data/menu header/la cas/talento humano/Plan de monitoreo del SIGEP/${item.filename}`;

    // Icono y Estilo según extensión
    ...
    // Insertamos dentro del div con id="plan-monitoreo-grid"
    const gridTag = 'id="plan-monitoreo-grid"';
  ...
  update(id, newData) {
    ...
    if (match) {
      ...
      let fileUrl = newData.filename
        ? `../../data/menu header/la cas/talento humano/Plan de monitoreo del SIGEP/${newData.filename}`
        : null;

      if (!name) {
        const nameMatch = /<h4>([\s\S]*?)<\/h4>/i.exec(oldCardHtml);
        name = nameMatch ? nameMatch[1] : "Sin nombre";
      }
      if (!fileUrl) {
        const hrefMatch = /href="([^"]*)"/i.exec(oldCardHtml);
        fileUrl = hrefMatch ? hrefMatch[1] : "#";
      }

      // Actualizar estilo si cambia extensión
  ...
  delete(id) {
    ...
    if (match) {
      ...
      const hrefMatch = /href="([^"]*)"/i.exec(cardHtml);
...
module.exports = new PlanMonitoreoModel();
