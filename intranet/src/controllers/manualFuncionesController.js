const model = require("../models/manualFuncionesModel");
...
const manualFuncionesController = {
  ...
  update: async (req, res) => {
    try {
      ...
      if (success) res.json({ status: "ok", message: "Manual actualizado" });
      else
        res
          .status(404)
          .json({ status: "error", message: "Manual no encontrado" });
    } catch (err) {
      res.status(400).json({ status: "error", message: err.message });
    }
  },
  ...
  delete: async (req, res) => {
    try {
      ...
      if (success) res.json({ status: "ok", message: "Manual eliminado" });
      else
        res
          .status(404)
          .json({ status: "error", message: "Manual no encontrado" });
...
module.exports = manualFuncionesController;
