...
// Ã¢ââ¬Ã¢ââ¬ SGI DIRECTORY SCAN: Procesos Misionales Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬Ã¢ââ¬
$SGI_MISIONAL_MAP = [
    'admin-recursos' => 'data/menu header/sgi/procesos misionales/Administracion de la Oferta de Recursos Naturales Renovables disponibles, Educacion Ambiental y Participacion Ciudadana',
    'vigilancia-control' => 'data/menu header/sgi/procesos misionales/Vigilancia, Seguimiento y Control Ambiental',
    'planeacion-ambiental' => 'data/menu header/sgi/procesos misionales/Planeacion y Ordenamiento Ambiental',
...
// 
//  13. MECI (Modelo EstÃÂ¡ndar de Control Interno)
// 
...
// ============================================================
//  GALERÍA FOTOGRÁFICA
//  GET    galeria                  → listar álbumes
//  GET    galeria/{album}          → listar fotos del álbum
//  POST   galeria/upload           → subir imagen(es) a un álbum
//  POST   galeria/album            → crear álbum
//  DELETE galeria/{album}/{id}     → eliminar foto
//  DELETE galeria/{album}          → eliminar álbum completo
// ============================================================

$GALERIA_BASE = __DIR__ . '/data/galeria';
if (!is_dir($GALERIA_BASE))
    mkdir($GALERIA_BASE, 0777, true);

// GET galeria — listar álbumes
if ($route === 'galeria' && $method === 'GET') {
    $albums = [];
    foreach (scandir($GALERIA_BASE) as $d) {
        if ($d === '.' || $d === '..')
            continue;
        $albumPath = $GALERIA_BASE . '/' . $d;
        if (!is_dir($albumPath))
            continue;
        $metaFile = $albumPath . '/metadata.json';
        $meta = file_exists($metaFile) ? read_json($metaFile) : [];
        // Count photos
        $photoCount = 0;
        $cover = null;
        foreach (scandir($albumPath) as $f) {
            ...
            if (is_file($albumPath . '/' . $f)) {
                $photoCount++;
                if (!$cover) {
                    $cover = WEB_BASE_PATH . 'data/galeria/' . rawurlencode($d) . '/' . rawurlencode($f);
                }
            }
        }
        $albums[] = [
    ...
    usort($albums, fn($a, $b) => strcmp($b['createdAt'], $a['createdAt']));
    out($albums);
}

// POST galeria/album — crear álbum
if ($route === 'galeria/album' && $method === 'POST') {
    auth();
    $in = body();
    $name = trim($in['name'] ?? '');
    if (!$name)
        out(['message' => 'Nombre requerido'], 400);
    $slug = preg_replace('/[^a-zA-Z0-9_-]/', '_', $name) . '_' . time();
    $albumPath = $GALERIA_BASE . '/' . $slug;
    if (!is_dir($albumPath))
        mkdir($albumPath, 0777, true);
    $meta = ['name' => $name, 'descripcion' => $in['descripcion'] ?? '', 'createdAt' => date('c')];
    write_json($albumPath . '/metadata.json', $meta);
    out(['success' => true, 'slug' => $slug, 'id' => md5($slug)], 201);
}

// GET galeria/{album} — listar fotos de un álbum
if (preg_match('#^galeria/([^/]+)$#', $route, $gm) && $method === 'GET') {
    $slug = $gm[1];
    $albumPath = $GALERIA_BASE . '/' . $slug;
    if (!is_dir($albumPath))
        out(['message' => 'Álbum no encontrado'], 404);
    $metaFile = $albumPath . '/metadata.json';
    $albumMeta = file_exists($metaFile) ? read_json($metaFile) : [];
    $photosMeta = $albumMeta['photos'] ?? [];
    $photos = [];
    foreach (scandir($albumPath) as $f) {
        if ($f === '.' || $f === '..' || $f === 'metadata.json')
            continue;
        ...
        $photos[] = [
            'id' => $id,
            'filename' => $f,
            'url' => WEB_BASE_PATH . 'data/galeria/' . rawurlencode($slug) . '/' . rawurlencode($f),
            'name' => $pm['name'] ?? pathinfo($f, PATHINFO_FILENAME),
            'caption' => $pm['caption'] ?? '',
    ...
    out(['album' => $albumMeta, 'photos' => $photos]);
}

// POST galeria/upload — subir foto(s) a un álbum
if ($route === 'galeria/upload' && $method === 'POST') {
    auth();
    $slug = $_POST['album'] ?? '';
    if (!$slug)
        out(['message' => 'Álbum requerido'], 400);
    $albumPath = $GALERIA_BASE . '/' . $slug;
    if (!is_dir($albumPath))
        out(['message' => 'Álbum no encontrado'], 404);
    ...
    $metaFile = $albumPath . '/metadata.json';
    $albumMeta = file_exists($metaFile) ? read_json($metaFile) : [];
    if (!isset($albumMeta['photos']))
        $albumMeta['photos'] = [];

    $uploaded = [];
    $allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml'];

    // Support both single file and multiple files (images[])
    $files = [];
        ...
        if (is_array($raw['name'])) {
            for ($i = 0; $i < count($raw['name']); $i++) {
                if ($raw['error'][$i] === UPLOAD_ERR_OK) {
                    $files[] = ['name' => $raw['name'][$i], 'tmp_name' => $raw['tmp_name'][$i], 'type' => $raw['type'][$i], 'size' => $raw['size'][$i]];
                }
            }
        } else {
            if ($raw['error'] === UPLOAD_ERR_OK) {
                $files[] = ['name' => $raw['name'], 'tmp_name' => $raw['tmp_name'], 'type' => $raw['type'], 'size' => $raw['size']];
            }
        }
    ...
    foreach ($files as $file) {
        if (!in_array($file['type'], $allowedTypes))
            continue;
        $clean = preg_replace('/[^a-zA-Z0-9._-]/', '_', $file['name']);
        $fname = time() . '_' . $clean;
        $dest = $albumPath . '/' . $fname;
        if (move_uploaded_file($file['tmp_name'], $dest)) {
            $id = md5($slug . '/' . $fname);
            $albumMeta['photos'][$fname] = [
            ...
            $uploaded[] = [
                'id' => $id,
                'filename' => $fname,
                'url' => WEB_BASE_PATH . 'data/galeria/' . rawurlencode($slug) . '/' . rawurlencode($fname),
                'name' => pathinfo($clean, PATHINFO_FILENAME),
            ];
        }
    }

    write_json($metaFile, $albumMeta);
    if (empty($uploaded))
        out(['message' => 'No se subió ninguna imagen'], 400);
    out(['success' => true, 'uploaded' => $uploaded], 201);
}

// DELETE galeria/{album}/{photoId} — eliminar foto
if (preg_match('#^galeria/([^/]+)/([a-f0-9]{32})$#', $route, $gm) && $method === 'DELETE') {
    auth();
    $slug = $gm[1];
    $photoId = $gm[2];
    $albumPath = $GALERIA_BASE . '/' . $slug;
    ...
    foreach (scandir($albumPath) as $f) {
        if ($f === '.' || $f === '..' || $f === 'metadata.json')
            continue;
        if (md5($slug . '/' . $f) === $photoId) {
            @unlink($albumPath . '/' . $f);
            unset($albumMeta['photos'][$f]);
            write_json($metaFile, $albumMeta);
            out(['success' => true]);
        }
    }
    out(['message' => 'Foto no encontrada'], 404);
}

// DELETE galeria/{album} — eliminar álbum completo
if (preg_match('#^galeria/([^/]+)$#', $route, $gm) && $method === 'DELETE') {
    auth();
    $slug = $gm[1];
    $albumPath = $GALERIA_BASE . '/' . $slug;
    if (!is_dir($albumPath))
