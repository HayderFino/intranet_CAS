document.addEventListener("DOMContentLoaded", () => {
  ...
  (() => {
    ...
    async function deleteItem(id) {
      if (
        !confirm("¡¿Eliminar este documento PCB? El archivo tambiÃÂ©n se borrarÃÂ¡.")
      )
        return;
      try {
  ...
  (() => {
    ...
    async function deleteItem(id) {
      if (!confirm("¡¿Eliminar este manual? El archivo tambiÃÂ©n se borrarÃÂ¡."))
        return;
      try {
  ...
  async function deleteRespelItem(id) {
    if (
      !confirm("¡¿Eliminar este registro? El archivo físico tambiÃÂ©n se borrarÃÂ¡.")
    )
      return;
    const section = respelCurrentSection.value;
    try {
  ...
  async function deleteBoletinItem(id) {
    if (
      !confirm("¡¿Eliminar este boletín? El archivo físico tambiÃÂ©n se borrarÃÂ¡.")
    )
      return;
    try {
  ...
  function applyUserPermissions(user) {
    ...
    const permissionMap = [
      ...
      // Galería
      { id: "nav-galeria", perm: p.galeria },

      // Procesos de Apoyo
      { id: "nav-procesos-apoyo", perm: p.procesos_apoyo },
    ];
    ...
    const groups = [
      ...
      {
        id: "nav-group-menu-lateral",
        items: ["nav-rua", "nav-respel", "nav-pcb", "nav-agenda", "nav-directorio"],
      },
      {
        id: "nav-group-git",
        items: ["nav-boletines", "nav-cita", "nav-sirh", "nav-revision-red", "nav-snif"],
      },
      {
